...
#    set_frame_range(0, -1, animation_armature)
    ...
    # Step 6: Extract mesh nodes for sampled frames
    print("Extracting mesh nodes...")
    
    # Find the mesh object associated with the armature
    mesh_objects = []
    for obj in bpy.context.scene.objects:
        if obj.type == 'MESH' and obj.parent == object_armature:
            mesh_objects.append(obj)
    
    if not mesh_objects:
        # If no child meshes, look for meshes with the same armature modifier
        for obj in bpy.context.scene.objects:
            if obj.type == 'MESH':
                for modifier in obj.modifiers:
                    if modifier.type == 'ARMATURE' and modifier.object == object_armature:
                        mesh_objects.append(obj)
                        break
    ...
    # Sample frames using np.linspace
    frames_to_sample = np.linspace(start_frame, end_frame, num=frames_n).astype(int)
    print(f"Sampling frames: {frames_to_sample}")
    
    # Extract mesh data for each frame
    for i, frame in enumerate(frames_to_sample):
        print(f"Extracting mesh for frame {frame} ({i+1}/{len(frames_to_sample)})")
        ...
        faces_filename = f"frame_{i:04d}_faces.npy"
        
        vertices_path = os.path.join(mesh_output_dir, frame_filename)
    ...
    metadata = {
        ...
        'end_frame': end_frame,
        'sampled_frames': frames_to_sample.tolist(),
        'num_vertices': len(vertices_array) if 'vertices_array' in locals() else 0,
        'num_faces': len(faces_array) if 'faces_array' in locals() else 0
    }
    
    metadata_path = os.path.join(mesh_output_dir, "metadata.json")
